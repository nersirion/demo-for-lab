...
def get_charts_names_voltage_gitt(df: pd.DataFrame) -> list:
    ...
    charts_vol = [
        (f"Cycle {cycle}", sheet, x, "Voltage")
        for (x, sheet) in sheets_and_x
        for cycle in cycles
    ]
    return charts_vol
...
class GittCharts(ChartMaker):
    ...
    def insert_data(self):
        for num_chart, name, sheet_name in self:
            ...
            num_chart = correct_num_chart(
                name, sheet_name, num_chart, self.config["n_cycles"]
            )
    ...
    def add_series(self, chart, name: str, sheet_name: str, num_chart: int):
        ...
        for num_series in range(self.get_number(name)):
            add_series_with_default_options(
                chart,
                self.generator,
                name,
                sheet_name,
            ...
            update_series(
                series, name, sheet_name, num_series, self.config, self.generator
            )
...
def add_series_with_default_options(
    ...
    categories, values = get_categories_and_values(
        name, sheet_name, num_series, num_chart, config_values
    )
    chart.add_series(
        {
...
def update_series(
    series, name: str, sheet_name: str, num_series: int, config_values: dict, generator
):
...
def get_categories_and_values(
    name: str, sheet_name: str, num_series: int, num_chart: int, config_values: dict
):
    row_start, row_end, col_start, col_end = get_row_col_index_values(
        name, num_series, num_chart, config_values
    )
    values = [sheet_name, row_start, col_start, row_end, col_end]
    row_start, row_end, col_start, col_end = get_row_col_index_categories(
        name, num_series, num_chart, config_values
    )
...
def get_row_col_index_values(
    name: str, num_series: int, num_chart: int, config_values: dict
) -> tuple:
    if name == "Diffrent Cycles":
...
def get_row_col_index_categories(
    name: str, num_series: int, num_chart: int, config_values: dict
) -> tuple:
    if name == "Diffrent Cycles":
...
def get_index(df: pd.DataFrame, i: int, groupby, unstack: str):
    index = df.groupby(groupby).apply(lambda x: x.index[i] + 1).unstack(unstack)
    return index
...
class MeanCharts(ChartMaker):
    ...
    def add_series(self, chart, name, sheet_name, num_chart: int, odd: bool):
        ...
        if odd:
            ...
            col = num_chart % 3 * 2 + 3
        first_index = (
            self.data["sheet_1"].groupby("sample").apply(lambda x: x.index[0] + 1)
        )
        last_index = (
            self.data["sheet_1"].groupby("sample").apply(lambda x: x.index[-1] + 1)
        )
        chart.add_series(
            {
                "categories": [
                    sheet_name,
                    first_index.loc[sample],
                    2,
                    last_index.loc[sample],
                    2,
                ],
                "values": [
                    sheet_name,
